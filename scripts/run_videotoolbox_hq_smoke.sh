...
client_root=""
for candidate in rustadmin-client rustdesk-client rustdesk; do
...
if [ -z "${client_root}" ]; then
  echo "error: could not find rustadmin-client or rustdesk-client or rustdesk under ${workspace_root}" >&2
  exit 1
fi
