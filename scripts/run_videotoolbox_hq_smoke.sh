...
client_root=""
for candidate in rustadmin rustadmin-client rustdesk-client; do
...
if [ -z "${client_root}" ]; then
  echo "error: could not find rustadmin, rustadmin-client, or rustdesk-client under ${workspace_root}" >&2
  exit 1
fi
